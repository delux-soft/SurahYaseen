...
import com.zohalapps.surahs.yaseen.rahman.alquran.audio.urdu.databinding.ItemCalenderCellBinding
...
class CalenderAdp(
    ...
    inner class CalenderVH(private val binding: ItemCalenderCellBinding) :
        ...
        fun bind(model: CalenderCellModel, position: Int) {
            if (model.isSelect) {
                binding.viewSelect.setBackgroundResource(drawable.bg_select)
            } else {
            ...
            if (model.georgianMonth == hGDate.getMonth() && model.georgianDay == hGDate.getDay() && model.georgianYear == hGDate.getYear()) {
                ...
                binding.viewBg.setBackgroundResource(drawable.bg_today)
            } else {
